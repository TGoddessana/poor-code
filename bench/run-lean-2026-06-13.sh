#!/usr/bin/env bash
# A/B arm: same 10 tasks, gpt-5.4-nano, on feat/lean-context (① env catch-all dropped
# + ② plan_gate advisory). Compare against today's nano baseline (2/10).
set -uo pipefail
...
export POOR_CODE_GIT_REF="feat/lean-context"     # ① + ② live here
export POOR_CODE_ADVISORY_GATES="1"              # ② enable plan_gate advisory (non-blocking)
...
echo "Starting lean-context run (nano, 10 tasks, ref=${POOR_CODE_GIT_REF}, advisory_gates=${POOR_CODE_ADVISORY_GATES})"
