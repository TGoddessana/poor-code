...
_SYSTEM = (
    ...
    "edit_scope.editable.\n"
    "5. A validation that starts a long-running process (a server) MUST stop it "
    ...
    "     validate: node server.js & sv=$!; sleep 2; curl -s localhost:3000/fib/10 | grep -qx 55; rc=$?; kill $sv 2>/dev/null; exit $rc\n"
    "  t3 package.json (main + scripts.start), depends on t2\n"
    "     validate: npm start & sv=$!; sleep 2; curl -s localhost:3000/fib/10 | grep -qx 55; rc=$?; kill $sv 2>/dev/null; exit $rc\n"
    "Call emit_plan once."
)
