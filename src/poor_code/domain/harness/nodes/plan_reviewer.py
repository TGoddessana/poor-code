...
from poor_code.domain.session.models import (
    CodeContext, Layer, Phase, Plan, Requirement, SessionState, TriggerKind,
    Verdict, VerdictKind,
)
...
_SYSTEM = (
    ...
    "6. COVERAGE GAP — an acceptance criterion has no task responsible for satisfying it.\n"
    "Validation is NOT your concern: the per-task Verifier owns the 'done' check by "
    "observation (grounded on the requirement's acceptance criteria) and the planner "
...
class PlanReviewer(AgentNode):
    ...
    requires = (Requirement, Plan, CodeContext)
    produces = ()
