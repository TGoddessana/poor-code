...
class PlainProseLLM:
    """Model that skips the interview_step tool and just asks in prose content."""
    def __init__(self, prose):
        self._prose = prose

    async def stream(self, messages, tools, response_format=None):
        from poor_code.provider.events import TextDelta, FinishedReason
        yield TextDelta(text=self._prose)
        yield FinishedReason(reason="stop")


@pytest.mark.asyncio
async def test_interviewer_treats_plain_prose_as_a_question():
    # The model bypassed structured output entirely and just typed the question.
    prose = '"원격 제어"라는 말이 너무 모호합니다. 어떤 시나리오인지 정확히 알려주세요.'
    node = Interviewer(PlainProseLLM(prose), project_map=_map())
    res = await node.run(NodeContext(state=_state(), cancel=asyncio.Event()))
    assert res.output is None
    assert res.query is not None
    assert res.query.prompt == prose
    assert res.query.kind is QueryKind.CLARIFY


@pytest.mark.asyncio
async def test_interviewer_schema_invalid_output_raises_with_raw_payload():
    # Without enforcement (FakeLLMClient declares no capabilities), a model that
    # violates the schema — here `query` as a bare string instead of the object —
    # must surface the FULL raw payload (Pydantic's own message truncates it).
